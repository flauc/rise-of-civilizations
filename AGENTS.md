...
- Keep admin panel tables clean with per-column filter controls rather than dense inline summary layouts; Reporting filters hide behind a Filters toggle; session rows open rich game detail on click (no separate View link).
...
- Featured civilization on the lobby/start screen uses a fixed top-to-bottom layout that must not shift when civ content changes.

## Learned Workspace Facts

- Admin analytics UI is a separate Vite app (typically `localhost:5174` or `5175`); it talks to the Bun game server API on `localhost:3001`. Overview is the live dashboard; Reporting has a Filters toggle plus fully clickable session rows with rich per-player drill-down (setup, civs, scores, cities, techs, legends); Games supports setup/player/score drill-down including AI opponents; map size labels use Small/Medium/Large not pixel dimensions.
- Guest sessions do not persist saved games; registered users are prompted to save on exit and saves survive server restarts; pre-password-validation accounts are grandfathered test users.
- Map lobby: **Random** is first (rolls full pool); **Continents (1–4)** randomizes landmass count plus fixed One / Two / Three / Four Continent presets; `generateMap` resolves layout from seed and the client shows it (e.g. `Random → Archipelago`).
- Trade routes pay flat base gold (not distance-based), prefer built roads on pathfinding and for tiered road bonuses, and support hub routing through owned ports so one click can reach an overseas city (client names via-cities); sealed routes recompute when a faster road path appears.
- Natural wonder placement uses terrain flags: `openOcean` wonders (e.g. Galápagos) require ocean tiles ringed by open ocean; `coastalWater` wonders (e.g. Great Barrier Reef) spawn on water tiles bordering land; `coastalFront` wonders land on coast frontage tiles.
...
- Native iOS/Android wrappers live in `mobile/` (Capacitor); macOS Sonoma needs Xcode 16.2 from developer.apple.com; iOS: `pod install` in `mobile/ios/App`, always open `App.xcworkspace`, `npm run build` in `mobile/` to refresh the simulator; Android Play Store: `npm run bundle:release` with R8 minify + native debug symbols in release `build.gradle` (release signing not yet wired in `build.gradle`); Android edge-to-edge uses `EdgeToEdge.enable()` in `MainActivity` (avoid deprecated status-bar APIs) for Play Console compliance.
...
- Built world wonders render on the map from `tile.wonder`; online `PlayerView` must expose wonder tiles (vector fallback when art is missing).
- Client legal pages (`/privacy`, `/terms`, support, delete-account) require no login; production web builds must set `VITE_BASE=/` so deep-link routes resolve `/assets/` and boot the legal overlay.
