...
- Game over screen: show stats summary, optional full-map explore, and a quit action; pin "Your empire" at the top; final standings use civ portraits with per-player victory progress toggled on row click (second click hides); View Map and Back to Summary must set `needsHudRender` so the victory overlay hides and the explore bar shows.
