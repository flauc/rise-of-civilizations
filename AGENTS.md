...
- Native iOS/Android wrappers live in `mobile/` (Capacitor); macOS Sonoma needs Xcode 16.2 from developer.apple.com; iOS: `pod install` in `mobile/ios/App`, always open `App.xcworkspace`, `npm run build` in `mobile/` to refresh the simulator; Android Play Store: `npm run bundle:release` (release signing not yet wired in `build.gradle`).
