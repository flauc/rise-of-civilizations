...
- Guest sessions do not persist saved games; registered users are prompted to save on exit and saves survive server restarts; pre-password-validation accounts are grandfathered test users. Registered accounts persist to `.roc-users.json` (or `ROC_USERS_FILE` / `ROC_DATA_DIR`) and to Postgres `roc_users` when `DATABASE_URL` is set; the server saves on register/delete, every 5 minutes, and on shutdown.
