...
- Do not commit or push git changes until the user explicitly asks; day-to-day work stays on `dev`, merge to `main` for mobile-release CI (`main` often lags `dev`, so version bumps on `dev` do not reach stores until merged; push to `main` does not deploy the web client; production web is a separate host deploy from `main`); GitHub Actions secrets go under repository Settings → Secrets and variables → Actions, not profile secrets.
