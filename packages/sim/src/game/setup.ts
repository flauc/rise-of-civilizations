...
export interface NewGameOptions {
  ...
  villages?: boolean | import("./state").VillageDensity;
...
export function createGame(opts: NewGameOptions = {}): GameState {
  ...
  for (let i = 0; i < count; i++) {
    ...
    players.push({
      ...
      researchQueue: [],
      scienceProgress: 0,
      governmentsResearched: new Set<string>(["chiefdom"]),
      researchingGovernment: null,
      civicsAdopted: new Set<string>(),
      slottedCivics: [],
      unrestTurns: 0,
      governmentChangedTurn: -Infinity,
      cultureProgress: 0,
      government: "chiefdom",
      faith: 0,
  ...
  placeFeatures(state, starts, activity, opts.villages ?? "medium");
