...
> **Audited 2026-06-19** against `packages/data/src/index.ts` (civ data), `packages/sim/src/game/leader-abilities.ts` (leader actives), and `packages/sim/src/game/content.ts` (units). This section records what of the **special units** and **special abilities** is actually wired into the sim versus what is still flavor text. Keep it in sync when civ content changes.

**All 82 civilizations in this doc exist as data entries.** Each `CivDef` carries: `id, name, leader, abilityName, abilityDesc, uniqueUnit (string), uniqueInfra (string), effects (flat bonuses), cityNames`. So every civ is selectable, names its cities historically, has a leader, and grants *some* mechanical bonus. The gap is between the **prose** (this doc's ability/UU/UI columns) and what the engine reads.

### ✅ Implemented

- **Civ ability — flat numeric portion only.** A civ's always-on identity bonus is implemented exclusively as its `effects` object (a `CivEffects`), merged in `civs.ts → playerEffects()`. The engine honors fields such as `yieldPercent`, `unitClassCombat`, `cavalry/naval/land/allUnitMovementBonus`, `goldPerWorkedDesert`, trade-route bonuses, tile-yield bonuses, `meleeVsCityBonus`, heal/maintenance modifiers, and the various `*CityYield` buckets. Example: Egypt *Iteru* = `yieldPercent.production +20`, Persia *Satrapies* = `gold +20 / melee +2`, Mali *Sahel Merchants* = `gold +10 / +2 gold per worked desert`.
...
- **Unique Infrastructure (UI).** Same as UUs — `uniqueInfra` is a string with no building/improvement implementation, no "replaces/augments" logic, no special yields. The whole "Unique Infrastructure" column is flavor.
...
- **The richer prose behaviors of most civ abilities.** Anything beyond a flat stat is *not* modeled. Representative gaps:
  ...
  - Babylon *Enuma Anu Enlil* "eurekas trigger instantly" → only `science +10%`.
  - Assyria *Treatises & Terror* "steal tech on city capture" → only as the *leader* ability, not the passive.
  - Rome *All Roads Lead to Rome* "free roads / free building in new cities" → not wired (no `newCityFreeBuilding` set on Rome).
  - Venice "cannot build settlers, buys cities"; Egypt "flood is beneficial"; Phoenicia "capital can move"; Aztec "captured units"; Inca "work mountains / terrace farms" → none modeled.
  - **Start bias** (the "Bias" column) — not consumed by map placement yet.

### Quick status table

| Layer | Status | Where |
|-------|--------|-------|
| Civ exists as selectable data (82) | ✅ | `packages/data/src/index.ts` |
...
## Historical civilizations not yet added (up to the Age of Exploration)

The 82 entries skew toward "headline" empires. Notable pre-~1500 CE civilizations/states **absent** from the roster, by region (★ = most glaring omission for the period):

- **Near East & Arabia:** ★ **Arabia / the Caliphate** (Rashidun–Umayyad–Abbasid — a defining power of the era, entirely missing), Mitanni, Urartu, Israel/Judah (Hebrews), Arameans, Nabataeans (Petra), Sabaeans/Saba & Himyar (Yemen).
- **Persia & Central Asia:** Greco-Bactria, Sogdia, Khwarazm, Samanids, Khazars, Uyghur Khaganate.
- **Africa:** Numidia (Masinissa), ★ **Fatimid / Ayyubid–Mamluk Egypt** (Saladin's world), Almoravids & Almohads (Berbers), ★ **Swahili Coast / Kilwa**, Benin (Edo), Ife/Yoruba, Kongo, Mutapa, Hausa city-states, Garamantes.
- **Europe:** ★ **Bulgaria** (First/Second Empire), Serbia, Bohemia, Old Swiss Confederacy, Crown of Aragon, Scotland, Gaelic Ireland, Normans/Kingdom of Sicily, Lombards, Visigoths, Vandals, Novgorod, Thracians/Dacians, Illyrians.
- **South & East Asia:** ★ **Indus Valley / Harappa**, Shang & Zhou China, ★ **Delhi Sultanate / Mughals** & **Vijayanagara**, Pallava/Pandya/Rashtrakuta, Champa, Funan, Sinhala (Sri Lanka), Khitan (Liao), Jurchen (Jin)/Manchu.
- **Steppe & Turkic:** Avars, Bulgars, Cumans/Kipchaks, Pechenegs, Golden Horde, Sarmatians/Alans, Crimean Khanate.
- **The Americas:** ★ **Andean pre-Inca** — Chavín, Moche, Nazca, Wari, Tiwanaku, **Chimú** (Chimor); Mixtec, Tarascan/Purépecha, Tlaxcala, Taíno (Caribbean), Calusa, Thule/Inuit.
- **Oceania:** Tonga (Tuʻi Tonga), Samoa, Fiji, Rapa Nui, Chamorro (Guam).

> Mughals and Vijayanagara straddle 1500; include them if "Age of Exploration" is read as the *start* (late 15th c.) rather than a hard cap.

➡️ **Proposed designs** (Civ Ability + Unique Unit + Leader Ability for 55 of these) are drafted in [CIVILIZATIONS-EXPANSION.md](CIVILIZATIONS-EXPANSION.md).
