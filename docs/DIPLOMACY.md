...
>
> **Status (v2 overhaul, 2026-06-20):** deals now use an explicit **propose → respond → finalize** lifecycle (`Proposal.status` = `pending`/`accepted`/`declined` + `reason`). A human proposer gets the AI's instant accept/decline (with a one-line reason) and must **finalize** an accepted offer; an AI-initiated deal or a **coercive tribute demand** concludes on accept without a finalize step. Tribute demands (`coercive`) are judged by **fear**: the AI yields only when it judges the demander to hold an overwhelming military edge (scaled by boldness), and a proud, strong AI may answer with war. AI war/peace/offer/demand decisions are now driven by per-civ **personalities** (`packages/data` `getPersonality` — aggression/boldness/loyalty/forgiveness/greed; hand-tuned for notable civs, deterministic-varied otherwise). A world **trade history** (`GameState.tradeHistory`) records every deal/gift/tribute/war/peace/denounce; the client diplomacy panel was rebuilt with a clear exchange summary, proposal inbox/outbox, **active agreements with expiry turns**, an **attitude breakdown**, and per-civ **history**. New command: `finalizeDeal`.
>
> **Valuation/AI tuning + AI-initiated trades (2026-06-20):** AIs no longer overpay for soft concessions — open borders are valued near-zero unless very friendly (or at war and needing passage), non-aggression pacts are valued mainly by **fear** (how much stronger the other civ is), and an AI won't spend more than ~25% of its treasury on soft concessions (full treasury only to buy peace or when it fears you), nor agree to pay gold/resources it lacks. Making a tribute demand always lowers the target's opinion (applied at demand time, before the accept/refuse outcome). Redundant offers (open borders / a pact already in force) are rejected by the sim and hidden in the deal-builder UI. **AIs now initiate trades** for things they need (`aiInitiateTrade`): a luxury they lack, or — when they have public works under way — a specialist; a wealthy civ pays gold-per-turn, a gold-poor civ barters a spare luxury. These arrive in the player's proposal inbox.
>
> **Extort-before-war (2026-06-20):** an AI with an **overwhelming** military edge (powerRatio ≥ 2) now **demands tribute before declaring war** rather than attacking outright — the threat of force is the lever. If the demand is refused (tracked by a hidden `__demandRefused` attitude marker, 8-turn TTL), a sufficiently **aggressive/warlike** civ then makes good on the threat and declares war; a peaceful-but-strong civ bluffs and won't actually invade. A truly warlike civ that already *despises* a beatable neighbour still declares war directly. The AI waits for a pending demand's answer before acting militarily.
