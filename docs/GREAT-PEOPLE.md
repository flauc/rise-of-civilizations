...
> ✅ **Status: IMPLEMENTED (2026-06-21), with simplifications — per-figure effects added 2026-07-08.** Both systems are live in `packages/sim` + `packages/data` with UI, AI, save/load, an in-game wiki category each, and generated portrait art. **What's faithful:** the full named rosters (every figure/hero below exists by name, era, and class/type), the point-pool → recruit → activate flow for Great People, and the faith-recruit → lifespan → aura flow for Legends. **What's simplified (read the per-section "Implementation" notes):**
> - **Great People effects are now per-FIGURE** (2026-07-08) — each named figure carries a distinct `GreatPersonGift` resolved in `great-person-gifts.ts` (Generals drill differently, Scientists burst differently, Merchants pay differently, etc.). **Prophets were already per-figure** (`ProphetGift`). Class-level `effect` hooks remain as metadata/fallback only. Auras, placed improvements (a General's Citadel), unit-attach, and Great Works / tourism beyond a single artefact slot are **not** fully implemented (the Writer/Artist/Musician classes are merged into one **Artist** culture class).
> - **Legends are recruited with FAITH** (a rising cost), not the varied per-hero paths (Faith/Culture/Conquest/Wonder/Quest) — those are flavour. Each hero reskins a base unit and carries a flat combat bonus + an adjacent-ally aura + an **era-scaled lifespan** (about 12 turns for the earliest heroes up to 25 for the longest-reigning, per `LegendDef.lifespan`; extendable per hero through kills, captures, pillage, tech/civic/religion milestones); the per-hero **signature active ability** is flavour (the base unit's own abilities apply). Hero budgets are balanced with the power model in [LEGENDS-BALANCE.md](LEGENDS-BALANCE.md). The optional **Mythic toggle** is not built.
> - **Era gating:** Great People and Legends only enter the recruitment pool once your empire has reached their era (`bronze_alloying` → Classical, `carburizing` → Medieval, `gunpowder` → Exploration). Earlier-era figures remain available once unlocked.
>
...
> **Implementation (`legends.ts`, `@roc/data` `LEGENDS`).** All heroes below exist with their era and type. They are recruited from the **⭐ Legends** panel by spending **faith** — the "Recruit via" column is flavour; the real cost is faith, rising **150 → 250 → 350 …** per hero. Each is **globally unique** while alive and **era-gated** (see above). On recruit, the hero spawns at one of your cities (naval heroes on adjacent water) as a unit reskinning a base type (`baseType`), with a flat **combat bonus** (slightly toned down at runtime), an **aura** giving adjacent friendly military +combat (strongest nearby aura only; no stacking), and a **15-turn base lifespan** (per-hero extensions via `legend-lifespan.ts` for kills, captures, pillage, tech/civic/revolution/religion events; a few — e.g. Joan of Arc — are *rechargeable* and return to the pool when they retire). On the map a hero shows a gold ring, a 👑 crown, and its name. Every hero's **signature power is now real** (2026-07-03): combat legends field curated/bespoke active-ability kits (`LEGEND_ABILITY_OVERRIDES`, content.ts), support legends exert passives while they live (`legend-passives.ts` per-turn ticks + `legend-effects.ts` presence effects) — see [UNIT-ABILITIES.md §9](UNIT-ABILITIES.md) for the full as-built table. Enabled by default; a per-game **Legends** toggle (lobby + `legendsEnabled`) switches the whole feature off. AI recruits heroes when it can afford them; an in-game **Wiki → Legends** category, generated **portraits** (`legends/<id>.png`) and **map unit tokens** (`units/<id>.png`) complete it. The optional **Mythic toggle** is not built.
