...
- **[CIVICS-AND-GOVERNMENTS.md](CIVICS-AND-GOVERNMENTS.md)** — the civics/government overhaul: a branching government tree + civics as expensive pros-and-cons cards (design, replaces the shipped flat civics/policies).
...
- ✅ **Religion** — a new ☮️ **faith** yield (Shrine/Temple buildings); accumulate faith to **found a religion** at a holy city, choosing up to 2 **beliefs** (Tithe, Scholarship, Warrior Code…) that merge into the founder's `playerEffects`. Religions **spread by proximity pressure** (strongest religion wins each city; holy cities anchored). A **religious victory** when one faith is the majority in every civ. Faith chip + Religion panel UI, city religion indicator, "Found Religion" smart suggestion; AI founds religions too.
- ⏳ Culture/tourism; trade routes; diplomacy/city-states; geodata-map integration.
